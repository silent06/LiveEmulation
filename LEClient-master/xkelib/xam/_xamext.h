...
extern "C" {
	...
	NTSYSAPI EXPORTNUM(333) int NTAPI RtlVsnprintf(IN CHAR* Buffer, IN int SizeInBytes, IN const CHAR* Format, IN va_list va);
	NTSYSAPI EXPORTNUM(37) VOID NTAPI HalReadWritePCISpace(IN BYTE Bus, IN BYTE Device, IN BYTE Function, IN DWORD offset, IN OUT PVOID pBuffer, IN DWORD cbBuffer, OUT BOOL isWrite);
	NTSYSAPI EXPORTNUM(315) VOID NTAPI XNetLogonGetExtendedStatus(DWORD* status, DWORD* errorStatus);
    ...
    typedef enum _XNotifyUITypes : DWORD {
   ...
   typedef enum _XNotifyUIPriorities : DWORD {
