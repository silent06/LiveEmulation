...
static UINT64 __declspec(naked) ExpansionCall(DWORD id, UINT64 r4 = NULL, UINT64 r5 = NULL, UINT64 r6 = NULL, UINT64 r7 = NULL) {
	__asm {
		li r0, 0x71
		sc
		blr
...
static UINT64 __declspec(naked) ExpansionInstall(DWORD PhysicalAddress, DWORD CodeSize) {
	__asm {
		li r0, 0x70
		sc
		blr
...
public:
	...
	static UINT64 PeekQWORD(UINT64 address);
	static UINT64 GetFuseLine(DWORD index);
	static DWORD PeekBytes(UINT64 address, PVOID buffer, DWORD length);
	static PBYTE GetHypervisorCPUKey();
	static PBYTE GetFuseCPUKey();
};

static BYTE HvPeekPokeExp[752] = {
