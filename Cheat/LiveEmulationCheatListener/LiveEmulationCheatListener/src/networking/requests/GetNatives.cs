...
namespace LiveEmulationCheatListener {
    class PacketGetNatives {
        public static void ServerPacketGetNatives(EndianReader reader, EndianWriter serverWriter, Header header, List<Log.PrintQueue> logId, string ip) {
            Log.Add(logId, ConsoleColor.Blue, "Command", "ServerPacketGetNatives", ip);
            ...
            Dictionary<int, ulong> _addresses = MySQL.GetAllNatives(header.m_Title);
        ...
        endshit:
            ...
            if (wasSuccessful) {
                ...
                Log.Add(logId, ConsoleColor.Cyan, "Information", string.Format("Sending {0} natives to client", count.ToString()), ip);
            }
